defmodule NewspaperWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed top-2 right-2 mr-2 w-80 sm:w-96 z-999 rounded-lg p-3 ring-1 z-9999",
...
def table(assigns) do
        ...
        <tr :for={{id, item} <- @rows} id={@row_id && @row_id.({id, item})} class="group hover:bg-zinc-50">
          ...
          <td :if={@action != []} class="relative w-14 p-0">
            <div class="relative whitespace-nowrap py-4 px-6 text-right text-sm font-medium">
              ...
              >
                <%= render_slot(action, @row_item.({id, item})) %>
              </span>
            </div>
          </td>
        </tr>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(NewspaperWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(NewspaperWeb.Gettext, "errors", msg, opts)
    end
  end
